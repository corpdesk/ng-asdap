...
export class LoginComponent implements OnInit {
  ...
  ngOnInit() {
    this.logger.info('ng-asdap-v2::LoginComponent::ngOnInit()/StorageType.CdObjId:', StorageType.CdObjId);
    ...
    this.logger.info('ng-asdap-v2::LoginComponent::ngOnInit()/filter:', filter);
      ...
      if (res.app_state.success === true) {
        ...
        */
        if (res.app_state.sess.cd_token !== null && res.app_state.success) {
          ...
          // this.svNav.nsNavigate(this,'/comm','message from ng-asdap')
        }
      } else {
  ...
  configPushPayload(triggerEvent: string, emittEvent: string, cuid: number): ICdPushEnvelop {
    this.logger.info('starting ng-asdap-v2::LoginComponent::configPushPayload()');
    ...
    // set recepient
    this.logger.info('ng-asdap-v2::LoginComponent::configPushPayload()/this.sidebarInitData:', JSON.stringify(this.sidebarInitData));
    this.logger.info('ng-asdap-v2::LoginComponent::configPushPayload()/this.sidebarInitData.value:', JSON.stringify(this.sidebarInitData.value));
    const uRecepient: ICommConversationSub = { ...users[0] }
    uRecepient.subTypeId = 7;
    this.logger.info('ng-asdap-v2::LoginComponent::configPushPayload()/uRecepient:', JSON.stringify(uRecepient));
    uRecepient.cdObjId = this.sidebarInitData.value
    envl.pushData.pushRecepients.push(uRecepient)

    this.logger.info('ng-asdap-v2::LoginComponent::configPushPayload()/envl:', JSON.stringify(envl));
